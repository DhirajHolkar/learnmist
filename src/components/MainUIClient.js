...
export default function MainUIClient({
  ...
  // fetch id ref to avoid race conditions when multiple index fetches run
  ...
  useEffect(() => {
    ...
    const currentFetchId = ++fetchIdRef.current; // mark this fetch
    ...
    (async () => {
      try {
        ...
        // only update if still the latest fetch
        if (fetchIdRef.current === currentFetchId) {
          setSidebar(concepts);
          setSelectedConceptSlug(concepts[0] && concepts[0].slug);
        }
  ...
  useEffect(() => {
    ...
    (async () => {
      try {
        ...
        // ensure the result still matches the current selectedConceptSlug
        if (!cancelled && doc && doc.slug === selectedConceptSlug) {
          setContent(doc);
          setLoading(false);
          return;
        }
        ...
        if (!cancelled) {
          if (fallbackDoc && fallbackDoc.slug === selectedConceptSlug) {
            setContent(fallbackDoc);
          } else {
            // If fallbackDoc doesn't match current slug, don't overwrite
            setContent(null);
          }
        }
      } catch (err) {
